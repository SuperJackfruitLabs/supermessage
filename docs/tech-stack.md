...
| Frontend (desktop) | **Svelte 5** (SPA mode, no SSR) | MIT | Tauri-community fit (GitButler precedent), lean compiled output |
...
| Frontend (iOS) | **Native SwiftUI** over UniFFI | — | Superseded the webview-skin plan — see "Corrected 19 Aug 2026" below |
...
- On **desktop**, UI skins are the only platform-branched layer. All logic, state and chat behaviour is shared.
- On **iOS**, the UI is a separate native app. What is shared is the core — and, since the view-model migration, every decision *about* what to draw: the render classification, rich-text parsing, the custom-event registry, room identity, previews and affordances all resolve in Rust and arrive on a `TimelineRow`/`RoomRow`.
...
## Corrected 19 Aug 2026 — iOS is native, not a webview skin

This file described a single Svelte UI wearing a Framework7 skin on the phone.
That is no longer the plan and several of its entries were simply false, so
they are removed rather than left to mislead:

- **Framework7 / Konsta as the mobile skin**, and decision 4 that chose it.
- **"iOS keyboard doesn't resize WKWebView — #1 'web tell' in a chat app"**,
  and the ~200 lines of objc2 it budgeted. SwiftUI does keyboard avoidance
  natively; so does the 16px focus-zoom rule that broke the Tauri build.
- **"Webview ceiling ≈ 85–90% native-adjacent."** There is no webview.
- **"Framework7 single-maintainer risk."** No dependency, no risk.
- **"IPC cost of streaming timelines to the webview."** iOS passes structs
  across UniFFI, not JSON across an IPC boundary.

What survives unchanged, and still binds: hard requirement #2 ("UI must feel
...
- [matrix-rust-sdk](https://github.com/matrix-org/matrix-rust-sdk) · [Tauri v2 docs](https://v2.tauri.app/) · [Bits UI](https://bits-ui.com/) · [virtua](https://github.com/inokawa/virtua)
