...
| 20 | **SSO/OIDC and registration** | Password only. Neither exists. | Currently costs nothing: the target homeserver advertises no SSO or OIDC and both discovery endpoints 404. It becomes urgent the moment matrix-authentication-service is deployed, or the moment a second homeserver is targeted. | M (blocked on server) |
...
| SSO/OIDC + registration | The `AuthProvider` seam made real; MSC3861 | Login flow branches, registration | **M** | Blocked on matrix-authentication-service being deployed. The seam is described in `AGENTS.md`; the code path is password-only. |
