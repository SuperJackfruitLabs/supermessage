...
| Reasoning placement, header (2026-09-24) | A finished turn's record ("Thought for 12s · 2 steps") is drawn inside the answer's row, between the agent's name and the text, as in ChatGPT, Claude and assistant-ui. The list header is your initial on the left and compose on the right. Invitations are a row in the list. Arrangement (Recent, Waiting) and Agent state moved to Account → Chats; Machine is dropped on phone because spaces cover it. Sign out, Leave room and Start over are drawn red. |
| Captions, notices, live card (2026-09-24) | Text typed with an attachment goes as its caption (MSC2530, `attachment_send(caption)`), on all three clients. A second message would reach a bridged agent mid-turn and be refused. Images show their captions. A one-line `m.notice` is a system line (`SystemKind::Notice`), so a bridge's refusal no longer reads as the agent speaking. The live card renders completed lines as markdown blocks (`StreamingRichView`), drops "Writing the answer" once text shows, and replaces the "is on it" pill. AgentPod also queues mid-turn messages (#557). |
