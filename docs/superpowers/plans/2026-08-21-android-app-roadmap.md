...
- **Who owns the reveal pacer?** On iOS the *view* does: `LiveTurnView.swift:25` holds `@State private var stream = StreamingText()` and feeds it from `.onChange(of: live.answer)`; `LiveStore` never touches it. The Kotlin `StreamingText` exists in `:kit` and its pacing rule is pinned there by `pacesTheRevealOverTicks` — the test iOS never had. What is *not* decided is who drives it on Android. A `:kit` task proposed giving `LiveStore` ownership and it was reverted as speculative: no consumer existed, and it bought the store a running coroutine job and a cancellation lifecycle. Decide it here, with the consumer in hand. If Compose wants the pacer tied to recomposition the way SwiftUI ties it to `@State`, that is the same answer iOS reached; if it wants the store to drive it, that is a small informed change.
- **How many decoded images can be in flight?** `:kit`'s `MediaCache` bounds by the UTF-8 length of the still-encoded `data:` URI, not by a decoded bitmap — it has no Android-graphics dependency to decode with, and deferring decode to the view is the right split. But it reuses iOS's 64 MiB constant against a much smaller unit, so the Android cache holds *far more* distinct images before evicting than iOS does. Nothing is wrong in `:kit`; the consequence lands here, in whatever decodes them. Decide the decode budget and eviction story in this phase rather than discovering it as an OOM on a low-memory device.
