 ...
 * arithmetic with no view toolkit in it — which means a test can exercise
 * *this* rather than a copy of it. A test that restates the rule it is
 * checking passes whatever the app does.
 */
object TimelineFollow {
    ...
    const val bottomThreshold: Float = 64f
    ...
    const val topThreshold: Float = 200f
    ...
    fun wantsOlderHistory(
        ...
        return canPaginate && !isPaginating && hasSettled && distanceFromTop < topThreshold
    }
     ...
     * timeline can do.
     */
    fun shouldRepin(distanceFromBottom: Float, grew: Boolean): Boolean =
     ...
     */
    fun shouldSettleAtBottom(previous: Int, next: Int, settled: Boolean): Boolean =
